...
mV <- glmer(total_fl_mass_ln ~ Veg_Mass_ln + (1|Plot) + (1|Scientific_Name) + (1|Population), family = gaussian, data = repro_fill_hi_n_narm_i)
...
# Left off here. i removed 14 influential points. That my have been too many. Check m_growth to see if removing points helped assumptions. 
# Now m_growth not a significant predictor compared to the model with just vegetative biomass.
# Consider adding in Population as fixed effect and looking at SPEI and precip. Remove low performing models, try more additive ones
...
boxplot(resid(m_growth) ~ Growth_Habit,
        data = repro_fill_hi_n_narm)
# No funnel shape but forbs have a larger spread

# 4 independence
boxplot(resid(m_growth) ~ Plot,
        data = repro_fill_hi_n_narm,
        las = 2,
        outline = FALSE)
abline(h = 0, lty = 2)
...
ranef_vals <- ranef(m_growth, condVar = TRUE)
...
car::vif(lm(total_fl_mass_ln ~ Growth_Habit * Veg_Mass_ln, data = repro_fill_hi_n_narm))
# all good because all less than 3

# 9 no influential points
library(influence.ME)
influence.ME::influence(m_growth, obs = TRUE)
infl <- influence(m_growth, obs = TRUE)
cd <- cooks.distance(infl)
cd_df <- data.frame(
  row = seq_along(cd),
  cooks_d = cd
)
p <- length(fixef(m_growth))   # number of fixed effects
n <- nobs(m_growth)
